...
namespace System
{
    ...
    public static partial class Converter
    {
        ...
        public static string ToSpaceTime(this TimeSpan ts)
        {
            //10<span class="plan_time_font">M</span>
            if ((int)ts.TotalSeconds < 60)
            {
                return (int)ts.TotalSeconds + "<span class='plan_time_font'>S</span>";
            }
            else if((int)ts.TotalMinutes < 60)
            {
                return (int)ts.TotalMinutes + "<span class='plan_time_font'>m</span>";
            }
            else if ((int)ts.TotalHours < 24)
            {
                return (int)ts.TotalHours + "<span class='plan_time_font'>H</span>";
            }
            else if ((int)ts.TotalDays < 30)
            {
                return (int)ts.TotalDays + "<span class='plan_time_font'>D</span>";
            }
            else if ((int)ts.TotalDays > 30)
            {
                return (int)ts.TotalDays % 30 + "." + (int)ts.TotalDays / 30 + "<span class='plan_time_font'>M</span>";
            }
            else if ((int)ts.TotalDays > 365)
            {
                return (int)ts.TotalDays % 365 + "." + (int)ts.TotalDays / 365 + "<span class='plan_time_font'>Y</span>";
            }
            else
            {
                return "";
